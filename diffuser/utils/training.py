...
from .arrays import batch_to_device, DEVICE as _DEFAULT_DEVICE

# On CUDA, use multiple workers + pin_memory for speed.
# On CPU/MPS, num_workers>0 can cause deadlocks on Mac; use 0.
...
class Trainer(object):
    def __init__(
        ...
        if train_test_split == 1:
            self.train_dataloader = cycle(torch.utils.data.DataLoader(
                self.dataset, batch_size=train_batch_size, num_workers=_NUM_WORKERS, shuffle=True, pin_memory=torch.cuda.is_available()
            ))
        else:
            ...
            self.train_dataloader = cycle(torch.utils.data.DataLoader(
                train_dataset, batch_size=train_batch_size, num_workers=_NUM_WORKERS, shuffle=True, pin_memory=torch.cuda.is_available()
            ))
            self.test_dataloader = cycle(torch.utils.data.DataLoader(
                test_dataset, batch_size=train_batch_size, num_workers=_NUM_WORKERS, shuffle=True, pin_memory=torch.cuda.is_available()
            ))
    ...
    def train(self):
        n_epochs = int(np.ceil(self.n_train_steps / self.n_steps_per_epoch))
        remaining_steps = self.n_train_steps
