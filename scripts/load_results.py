...
exp = os.environ.get('RESULT_EXP', 'avoiding-d3il')
...
# Plot results for the variants that are actually present in the current eval config.
candidate_groups = [
    ['dpcc-r', 'dpcc-t', 'dpcc-c'],
    ['dpcc-r-tightened', 'dpcc-t-tightened', 'dpcc-c-tightened'],
    projection_variants,
]

for variants in candidate_groups:
    variants = [variant for variant in variants if variant in sr_goal_all]
    if not variants:
        continue
    variants_labels = [variant.replace('_', '-').upper() for variant in variants]
    ...
    suffix = 'tightened' if 'tightened' in variants[0] else 'current'
    plt.savefig(f'success_rates_{suffix}.png')
    plt.show()
    ...
    fig.tight_layout()
    plt.savefig(f'timesteps_{suffix}.png')
    plt.show()
