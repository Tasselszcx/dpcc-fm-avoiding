...
**双骨干**全量(3 种子 x 3 场景 x 50 试验,晚投影调度),计时为空机单进程:

| 骨干 | goal% | goal+cons% | viol steps | time/step |
| --- | ---: | ---: | ---: | ---: |
| FM   `cvxpyqp` | 77.3 | **74.0** | 0.4 | 0.542s |
| DDPM `cvxpyqp` | 73.1 | **72.7** | 0.1 | 0.616s |

**质量与 SLSQP 基线一致(近零违反,goal+cons 74.0 / 72.7 对比 SLSQP 72.0 / 71.3),证明 DPCC
的有效性与求解器无关。** 代价是约为 SLSQP 的 4 倍(≈0.55-0.62s vs ≈0.14s),因为每次投影要做
5 次 SCP 锥子问题求解。凸 QP 路径是有用的正确性交叉验证,但在本任务上不是速度优势;默认仍用 SLSQP。
...
goal 很高(约 99%),但**满足约束仅约 33%,每条 rollout 有约 27-30 个违反步**——基本和完全
不投影一样不安全(diffuser:约 28 个违反步)。软引导把轨迹往可行方向"推",但从不保证可行,
因此在核心安全指标上远差于硬投影。这反过来证明了:真正让 DPCC 起作用的是把轨迹*强制投影到*
可行集,而不仅仅是惩罚不可行。晚投影调度(`lateproj20`)仍是推荐的 FM 配置。
